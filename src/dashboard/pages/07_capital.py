...
with c1:
    st.metric("Operating CF", f"{latest['operating_activity']:,.0f}")

with c2:
    st.metric("Investing CF", f"{latest['investing_activity']:,.0f}")

with c3:
    st.metric("Financing CF", f"{latest['financing_activity']:,.0f}")
...
fig = px.bar(
    company_df, x="year", y="capex_cr", text="capex_cr", title="Capital Expenditure"
)
